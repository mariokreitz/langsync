...
<Callout title="Why Corepack?">
  Corepack reads the `packageManager` field from your `package.json` and
  provisions the exact pnpm/yarn version. It's bundled with Node 22, so no
  third-party setup action is required.
</Callout>
